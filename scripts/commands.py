...
@command
def tpa(script, a, b=None):
    """Teleport to user with no elevated permissions."""
    entity = script.connection.entity
    ...
    script.loop.call_later(0.1, send_reset_packet)
    return 'Poof! Teleported to %r ' % (player.name)
...
def spawn(script):
    ...
    pos = qvec3(32804, 32803, 0) * constants.CHUNK_SCALE
    ...
    script.loop.call_later(0.1, send_reset_packet)
    return 'Poof! Teleported to the spawn.'
